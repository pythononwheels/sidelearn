...
| `richdict-<learn>-de.json` | `{ word: { b, s:[{t,p,ex,exd}], alt } }` — rich, leveled learner dictionary (1–3 senses, part of speech, example FR/DE) | 2 |

`richdict-*-de.json` is generated by `scripts/build-richdict.mjs` (needs a Gemini
key): the German **meanings are grounded in FreeDict** (`dict-<learn>-de.json`),
while part-of-speech, the example sentences (learning language + German), and the
learner-frequency sense ordering are produced by the LLM. It powers the Wörterbuch,
the word popover (offline-first) and the multiple-choice Vokabeltest.
